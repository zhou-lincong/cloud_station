...
func TestBucketList(t *testing.T) {
	...
	if err != nil {
		t.Log(err, ": ListBuckets err。获取Bucket列表失败")
	}
...
func TestUploadFile(t *testing.T) {
	//返回一个*oss.Bucket对象，获得这个对象就可以在这个对象上进行操作了
	//跟在页面点到某个具体的bucket里面去操作效果是一样的
	...
	// mydir/test.go, 阿里云oss server会自动创建一个mydir目录， mkdir -pv
